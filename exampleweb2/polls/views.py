...
def vote(request, question_id):
    ...
    choice_id = int(request.POST['choice']) # request.POST['name 속성의 값'] : POST로 전송된 요청 데이터를 읽는 방법 -> 문자열
    ...
    choice.save() # 데이터베이스에 변경 내용 저장

    # 투표 결과 화면으로 이동 (redirect 이동 : 브라우저로 응답을 보내는데 이 응답이 즉시 다시 서버로 새로운 요청을 보내도록 처리)
    return HttpResponseRedirect(reverse("polls:results", args=(question_id, ))) # reverse : urls.py에 등록된 요청경로를 가져와서 사용
