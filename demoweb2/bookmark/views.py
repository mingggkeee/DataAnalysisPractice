...
class BookmarkLV(ListView):
    ...
    # 1. 목록 조회
    # 2. 템플릿으로 이동 (템플릿에서 사용할 수 있도록 데이터 전달)
    model = Bookmark 
...
class BookmarkDV(DetailView):
    ...
    # template_name = 'bookmark_detail.html' : 지정하지 않을 경우 bookmark_detail.html
...
class BookmarkCreateView(LoginRequiredMixin, CreateView):  # LoginRequiredMixin : 로그인 하지 않은 사용자의 요청인 경우 로그인 화면으로 이동 처리하는 기능 제공
    ...
    # template_name = 'bookmark/bookmork_form.html'
...
class BookmarkChangeLV(LoginRequiredMixin, ListView): 
    ...
    def get_queryset(self): # 이 뷰가 templates에게 전달하는 데이터 집합을 생산하는 메서드 ( 이 메서드를 정의하지 않으면 Bookmark.objects.all() )
...
class BookmarkDeleteView(OwnerOnlyMixin, DeleteView): # DeleteView : pk로 전달되는 데이터를 사용해서 모델의 데이터를 (여기서는 Bookmark ) 삭제하는 기능 제공
    model = Bookmark 
    success_url = reverse_lazy('bookmark:index')